...
$matkul = mysql_query("SELECT * FROM matkul WHERE id_user = '$_SESSION[id]' ") or die(mysql_error());
		...
		$matkuli = mysql_query("SELECT * FROM matkul WHERE id_matkul = '$_SESSION[id]' ") or die(mysql_error());
		$tampil_matkuli = mysql_fetch_array($matkuli);
...
<select name="id_matkul">  
 		...
 		<option value="<?php echo $tampil_matkul['nama_matkul'];?>"> <?php echo $tampil_matkul['id_matkul'];?> </option>

        </select>

<!-- <h1>Daftar Nilai <?php echo $tampil_matkul['nama_matkul'];?></h1> -->
 ...
 ?>	
   ...
   ?>
	...
	<tr>
    	...
    	<tr><td colspan='2' align='center' height="68"><input type="submit" name="submit" value="INPUT"/></td></tr>
</table>
</form>
